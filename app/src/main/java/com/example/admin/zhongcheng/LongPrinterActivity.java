...
public class LongPrinterActivity extends Activity implements OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.longprinter_result);
        ...
        String content = prf.getString("频繁调用打印", "");
        editText.setText(content);
        int tag=prf.getInt("tag_longprinter",6);
        if (tag==1){
            btn_pass.setBackgroundColor(Color.GREEN);
            btn_fail.setBackgroundColor(Color.GRAY);
    ...
    wifi=editText.getText().toString();
        ...
        editor.putString("频繁调用打印", editText.getText().toString());
        editor.putInt("tag_longprinter",flag);
        editor.commit();
