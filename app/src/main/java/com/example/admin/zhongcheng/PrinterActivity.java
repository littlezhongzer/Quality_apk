...
public class PrinterActivity extends Activity implements OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.printer_result);
        ...
        String content = prf.getString("打印机", "");
        editText.setText(content);
        int tag=prf.getInt("tag_printer",6);
        if (tag==1){
            btn_pass.setBackgroundColor(Color.GREEN);
            btn_fail.setBackgroundColor(Color.GRAY);
    ...
    wifi=editText.getText().toString();
        ...
        editor.putString("打印机", editText.getText().toString());
        editor.putInt("tag_printer",flag);
        editor.commit();
