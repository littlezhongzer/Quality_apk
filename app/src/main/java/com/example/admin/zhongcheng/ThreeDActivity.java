...
public class ThreeDActivity extends Activity implements OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.threed_result);
        ...
        String content = prf.getString("3D测距", "");
        editText.setText(content);
        int tag=prf.getInt("tag_threed",6);
        if (tag==1){
            btn_pass.setBackgroundColor(Color.GREEN);
            btn_fail.setBackgroundColor(Color.GRAY);
    ...
    wifi=editText.getText().toString();
        ...
        editor.putString("3D测距", editText.getText().toString());
        editor.putInt("tag_threed",flag);
        editor.commit();
