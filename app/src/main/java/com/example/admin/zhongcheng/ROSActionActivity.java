...
public class ROSActionActivity extends Activity implements OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.rosaction_result);
        ...
        String content = prf.getString("底盘执行动作", "");
        editText.setText(content);
        int tag=prf.getInt("tag_rosaction",6);
        if (tag==1){
            btn_pass.setBackgroundColor(Color.GREEN);
            btn_fail.setBackgroundColor(Color.GRAY);
    ...
    wifi=editText.getText().toString();
        ...
        editor.putString("底盘执行动作", editText.getText().toString());
        editor.putInt("tag_rosaction",flag);
        editor.commit();
