...
public class ScreenTestActivity extends Activity implements OnClickListener {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.screentest_result);
        ...
        String content = prf.getString("屏幕显示测试", "");
        editText.setText(content);
        int tag=prf.getInt("tag_screentest",6);
        if (tag==1){
            btn_pass.setBackgroundColor(Color.GREEN);
            btn_fail.setBackgroundColor(Color.GRAY);
    ...
    wifi=editText.getText().toString();
        ...
        editor.putString("屏幕显示测试", editText.getText().toString());
        editor.putInt("tag_screentest",flag);
        editor.commit();
